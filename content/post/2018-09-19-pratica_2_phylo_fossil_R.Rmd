...
phylo.list.30 <- sim.bd.age(age = 30, numbsim = 1000, lambda = 0.2, mu = 0, complete = TRUE) ## bem lento
...
phylo.list.bd.30 <- sim.bd.age(age = 30, numbsim = 200, lambda = 0.4, mu = 0.2, complete = TRUE) ## beeeem lento

## Obter o número de espécies para as filogenias moleculares que representam o momento final da simulação
ntip.bd.30 <- unlist(lapply(phylo.list.bd.30, function(x){ifelse(class(x) == "phylo", Ntip(drop.fossil(x)), 0)})) ## lento
```
...
sum(unlist(lapply(phylo.gamma.null.viper, gammaStat)) <= gammaStat(viper.tree))/1000
```
